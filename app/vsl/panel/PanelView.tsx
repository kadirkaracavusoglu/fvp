import { PANEL_RANGES, type FunnelStep, type VslPanelData } from "@/lib/vsl-panel";
...
function Kpi({ label, value, sub }: { label: string; value: string; sub?: string }) {
  return (
    <div className="rounded-xl border border-[#e6e8ea] bg-white p-4">
      ...
      <div className="mt-1 text-3xl font-bold tabular-nums text-[#0d204d]">{value}</div>
...
function FunnelCard({ title, steps }: { title: string; steps: FunnelStep[] }) {
  ...
  return (
    <section className="rounded-xl border border-[#e6e8ea] bg-white p-5">
      <h2 className="text-sm font-bold uppercase tracking-wide text-[#0d204d]">{title}</h2>
      {empty ? (
        <p className="mt-5 rounded-lg bg-[#f4f6f9] px-4 py-6 text-center text-sm text-gray-400">Bu aralıkta veri yok.</p>
      ) : (
        <div className="mt-4 space-y-3">
          {steps.map((step, i) => {
            const width = top ? Math.max(2, Math.round((step.count / top) * 100)) : 0;
            return (
              <div key={step.key}>
                <div className="mb-1 flex items-baseline justify-between gap-3 text-sm">
                  <span className="font-medium text-[#0d204d]">{step.label}</span>
                  <span className="shrink-0 tabular-nums text-gray-400">
                    <strong className="text-[#0d204d]">{step.count}</strong>
                    {i > 0 && <span className="ml-2 font-semibold text-[#0d204d]">{pct(step.pct)}</span>}
                  </span>
                </div>
                <div className="h-2 overflow-hidden rounded-full bg-[#f4f6f9]">
                  <div className="h-full rounded-full bg-[#0d204d]" style={{ width: `${step.count ? width : 0}%` }} />
                </div>
                {i > 0 && step.pctPrev != null && step.pctPrev !== step.pct && (
                  <p className="mt-0.5 text-xs text-gray-400">önceki adımın {pct(step.pctPrev)}&apos;i</p>
                )}
              </div>
            );
          })}
...
export function PanelView({ data, logout }: { data: VslPanelData; logout: () => void }) {
        ...
        <header className="mb-6 flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-sm text-gray-400">
              {data.startDate} → {data.endDate} · son güncelleme {fmtDate(data.generatedAt)}
            </p>
          </div>
        ...
        {!data.ok ? (
          ...
          <>
            <div className="mb-4 grid grid-cols-2 gap-3 lg:grid-cols-4">
              <Kpi label="VSL ziyaret" value={String(data.kpi.visits)} sub="vsl_optin_view" />
              <Kpi label="Opt-in" value={String(data.kpi.optins)} sub={`ziyaret → opt-in ${pct(data.kpi.optinRate)}`} />
            ...
            <div className="grid gap-4 lg:grid-cols-[1.25fr_1fr]">
              ...
              <section className="rounded-xl border border-[#e6e8ea] bg-white p-5">
                <h2 className="text-sm font-bold uppercase tracking-wide">Takip Sağlığı</h2>
                <div className="mt-4 space-y-2">
                  {data.trackingHealth.map((item) => (
                    <div key={item.label} className="flex items-center justify-between gap-3 rounded-lg bg-[#f4f6f9] px-3 py-2 text-sm">
                      <span>{item.label}</span>
                      <span className={item.state === "ok" ? "font-semibold text-[#157347]" : "font-semibold text-[#b45309]"}>
                        {item.value}
                      </span>
                    </div>
                  ))}
